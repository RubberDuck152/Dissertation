...
public class TerrainSpawnerWall : MonoBehaviour
{
...
#pragma warning restore CS0108 // Member hides inherited member; missing new keyword
    ...
    public GameObject newWallObject;
            ...
            if (canSpawn == true)
            {
                if (Physics.Raycast(camera.transform.position, camera.transform.TransformDirection(Vector3.forward), out RaycastHit hitinfo, maxRayDistance, layerCheck))
                {
                    ...
                    if (Vector3.Distance(teleportSphere.transform.position, player.transform.position) > minDistance)
                    {
                        ...
                        {
                            newWallObject = Instantiate(wallObject);
                            newWallObject.transform.position = teleportSphere.transform.position + new Vector3(0.0f, newWallObject.transform.localScale.y / 2.0f, 0.0f);
